...
export const findFileById = async (idProcessesFile: number, resulting?: boolean): Promise<Result<ProcessesFile>> => {
    try {

        const res = await api.processManagement.get<ProcessesFile> (
            `/processesFile/findFileById/${idProcessesFile}/${resulting ? 'resulting' : 'original'}`,
        );
...
export const findAllPaged = async (pagination?: Pagination, nameOrRecord?: string): Promise<Result<any>> => {

    try {
        const res = await api.processManagement.get<ProcessesFile[]>(`/${processFileUrl}/findAllPaged`, {
            params: {
...
export const findAllItemsPaged = async (idProcessesFile: number, pagination?: Pagination): Promise<Result<any>> => {

    try {
        const res = await api.processManagement.get<ProcessesFileItem[]>(`/${processFileUrl}/findAllItemsPaged?idProcessesFile=${idProcessesFile}`, {
            params: {
